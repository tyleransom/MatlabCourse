...
y = activityt(:);
y = (y==2);
[b_no_het,~,stats]=glmfit(X,y,'binomial','link','logit','constant','off');
like_no_het = sum(y.*log(sigmoid(X*b_no_het))+(1-y).*log((1-sigmoid(X*b_no_het))));
...
b_disc_RE                             = fminsearch('logit_het_disc',[bstart;.4*rand],options,X,d);
b_disc_RE                             = fminsearch('logit_het_disc',b_disc_RE,options,X,d);
b_disc_RE                             = fminsearch('logit_het_disc',b_disc_RE,options,X,d);
b_disc_RE                             = fminsearch('logit_het_disc',b_disc_RE,options,X,d);
[b_disc_RE,l_disc_RE,~,~,~,h_disc_RE] = fminunc   ('logit_het_disc',b_disc_RE,options,X,d);
SE_b_disc_RE = sqrt(diag(inv(h_disc_RE)));
results_disc_RE = [b_disc_RE SE_b_disc_RE];
%% Problem 3(c)
[b_RE]                 = fminsearch('logit_het_grid',[bstart;.4*rand],options,X,d);
[b_RE,l_RE,~,~,~,h_RE] = fminunc   ('logit_het_grid',b_RE,options,X,d);
