function like = logit_het_grid_prime(b,X,y)
%LOGIT_HET_GRID Computes random effects for a binary logit
...
like = -sum(log(sum(squeeze(prod(sigmoid(Xb1+grid).^((y==1)) .* (1-sigmoid(Xb1+grid)).^(y==0),2)).*normpdf_weights.*simpson_weights,2)),1);
end
