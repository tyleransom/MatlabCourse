...
%% Problem 3(c)
[b_RE]                 = fminsearch('probit_het_grid_prime',[bstart;.4*rand],options,X,d);
[b_RE,l_RE,~,~,~,h_RE] = fminunc   ('probit_het_grid_prime',b_RE,options,X,d);
SE_b_RE = sqrt(diag(inv(h_RE)));
results_RE = [b_RE SE_b_RE];
results_compare=[[b_no_het;NaN;NaN] [b_disc_RE;l_disc_RE] [b_RE;l_RE]];
save estimation_results_prime int_hat_quad quad_time int_hat int_time n results_no_het results_disc_RE results_RE results_compare
